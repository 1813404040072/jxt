...
export class OrderService {
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http
      .get<IOrder[]>(this.resourceUrl + '/query', { params: options, observe: 'response' })
  ...
  protected convertDateFromClient(order: IOrder): IOrder {
    const copy: IOrder = Object.assign({}, order, {
      ...
      createdDate: order.createdDate != null && order.createdDate.isValid() ? order.createdDate.format(DATE_FORMAT) : null,
      modifiedDate: order.modifiedDate != null && order.modifiedDate.isValid() ? order.modifiedDate.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((order: IOrder) => {
        ...
        order.createdDate = order.createdDate != null ? moment(order.createdDate) : null;
        order.modifiedDate = order.modifiedDate != null ? moment(order.modifiedDate) : null;
      });
    }
    return res;
  }
