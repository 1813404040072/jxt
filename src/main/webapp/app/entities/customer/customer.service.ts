...
export class CustomerService {
  ...
  protected convertDateFromClient(customer: ICustomer): ICustomer {
    const copy: ICustomer = Object.assign({}, customer, {
      createdDate: customer.createdDate != null && customer.createdDate.isValid() ? customer.createdDate.format(DATE_FORMAT) : null,
      modifiedDate: customer.modifiedDate != null && customer.modifiedDate.isValid() ? customer.modifiedDate.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((customer: ICustomer) => {
        ...
        customer.modifiedDate = customer.modifiedDate != null ? moment(customer.modifiedDate) : null;
      });
    }
    return res;
  }
